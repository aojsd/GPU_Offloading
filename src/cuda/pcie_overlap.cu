/**
 * @file pci_overlap_matmul.cu
 * @brief Empirically tests two strategies for overlapping PCIe transfers with computation
 * for a large matrix multiplication problem (A * B = C) where matrix A is partially
 * offloaded to host DRAM.
 *
 * Shapes: A[N x H], B[H x S], C[N x S]
 *
 * This version uses a pure CUDA Stream-based approach for both tests and includes
 * a fast, GPU-based verification function.
 *
 * Design 1 (Explicit Overlap):
 ...
 * - An async PCIe copy is launched on the transfer stream.
 ...
 *
 * Design 3 (Unified Virtual Memory - UVM):
 * - Matrices are allocated with `cudaMallocManaged`.
 * - In each trial, an async prefetch of the offloaded data is launched concurrently
 ...
 * make
 * (Optional: specify GPU architecture, e.g., `make ARCH=sm_86`)
 *
 * Compilation (manual):
 * nvcc -O3 -arch=sm_70 pci_overlap_matmul.cu -o pci_overlap_matmul
 * (Adjust -arch=sm_XX to your GPU's compute capability)
 *
 * ---
 *
 * Usage:
 * ./pci_overlap_matmul [options]
...
void runSingleKernelTest(int N, int H, int S, int trials, bool use_uvm, int deviceId) {
    ...
    std::cout << "GPU Throughput (GB/s): " << std::setw(8) << A_size / (avg(kernel_times) * 1e6) << "\n";
    std::cout << "Total Kernel Time:    " << std::setw(8) << avg(kernel_times) << " ms\n";
...
void runExplicitOverlapTest(int N, int H, int S, float offload_ratio, int trials) {
    std::cout << "\n--- Running Design 1: Explicit Overlap (CUDA Graph Version) ---\n";
    ...
    std::cout << "GPU Throughput (GB/s): " << std::setw(8) << (A_size / (1e6 * (avg(compute1_times) + avg(compute2_times)))) << " GB/s\n";
    std::cout << "--------------------------------------\n";
    std::cout << "Total Kernel Time:    " << std::setw(8) << avg(total_times) << " ms\n";
    std::cout << "Total Compute Time = " << avg(compute1_times) + avg(compute2_times) << " ms\n";
...
void runUvmTest(int N, int H, int S, float offload_ratio, int trials, int deviceId) {
    ...
    if (SERIALIZE_KERNELS) {
        std::cout << "\n--- Running Design 3: UVM with Serial Kernels (Multi-Stream) ---\n";
    } else {
        std::cout << "\n--- Running Design 3: UVM with Concurrent Kernels (Multi-Stream) ---\n";
    }
    ...
    CHECK_CUDA(cudaMallocManaged(&B, (size_t)H * S * sizeof(float)));
    CHECK_CUDA(cudaMallocManaged(&C, (size_t)N * S * sizeof(float)));
...
void print_usage(const char* prog_name) {
    ...
    std::cerr << "  --uvm                              Use Unified Virtual Memory (Design 3) instead of explicit overlap.\n";
    std::cerr << "  -N, --N, --rows <int>              Number of rows for matrix A. (Default: 4096)\n";
    std::cerr << "  -H, --H, --hidden_dim <int>        Number of columns for A / rows for B. (Default: 2048)\n";
    std::cerr << "  -S, --S, --cols <int>              Number of columns for matrix B. (Default: 2048)\n";
    std::cerr << "  -r, --ratio, --offload_ratio <f>   Fraction of matrix A to offload to host DRAM (0.0 to <1.0). (Default: 0.2)\n";
    std::cerr << "  -t, --trials <int>                 Number of timed trials to run. (Default: 10)\n";
    std::cerr << "  -d, --device <id>                  ID of the GPU device to use. (Default: 0)\n\n";
}

// Simple command line parser with error handling
bool parse_args(int argc, char** argv, int& N, int& H, int& S, float& ratio, int& trials, bool& use_uvm, int& device_id) {
...
int main(int argc, char** argv)
{
    ...
    if (!parse_args(argc, argv, N, H, S, offload_ratio, trials, use_uvm, device_id)) {
    ...
    std::cout << "  Mode:          " << (use_uvm ? "UVM (Design 3)" : "Explicit Overlap (Design 1)") << "\n";
